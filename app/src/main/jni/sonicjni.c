...
jlong Java_fm_player_mediaplayer_player_Sonic_initNative(JNIEnv *env, jobject thiz, jint sampleRate,
		jint channels) {
...
void Java_fm_player_mediaplayer_player_Sonic_closeNative(JNIEnv *env, jobject thiz, jlong sonicID) {
...
jboolean Java_fm_player_mediaplayer_player_Sonic_putBytesNative(JNIEnv *env, jobject thiz,
		jlong sonicID, jbyteArray buffer, jint lenBytes) {
	sonicInst inst = getInst(sonicID);
	sonicStream stream = inst->stream;
...
// TODO: deal with case where remainingBytes is not 0.
	...
	(*env)->GetByteArrayRegion(env, buffer, 0, lenBytes,
			(jbyte *) inst->byteBuf);
...
jint Java_fm_player_mediaplayer_player_Sonic_receiveBytesNative(JNIEnv *env, jobject thiz,
		jlong sonicID, jbyteArray ret, jint lenBytes) {
	sonicInst inst = getInst(sonicID);
	sonicStream stream = inst->stream;
	int available = sonicSamplesAvailable(stream) * sizeof(short)
			* sonicGetNumChannels(stream);
...
void Java_fm_player_mediaplayer_player_Sonic_setPitchNative(JNIEnv *env, jobject thiz, jlong sonicID,
		jfloat newPitch) {
...
jfloat Java_fm_player_mediaplayer_player_Sonic_getPitchNative(JNIEnv *env, jobject thiz,
		jlong sonicID) {
	sonicStream stream = getInst(sonicID)->stream;
	LOGV("Reading pitch");
...
// decrease the pitch.
void Java_fm_player_mediaplayer_player_Sonic_setRateNative(JNIEnv *env, jobject thiz, jlong sonicID,
		jfloat newRate) {
	sonicStream stream = getInst(sonicID)->stream;
...
jfloat Java_fm_player_mediaplayer_player_Sonic_getRateNative(JNIEnv *env, jobject thiz,
		jlong sonicID) {
	sonicStream stream = getInst(sonicID)->stream;
	LOGV("Reading rate");
...
jint Java_fm_player_mediaplayer_player_Sonic_getSampleRateNative(JNIEnv *env, jobject thiz,
		jlong sonicID) {
	sonicStream stream = getInst(sonicID)->stream;
...
// Set the sample rate.
void Java_fm_player_mediaplayer_player_Sonic_setSampleRateNative(JNIEnv *env, jobject thiz,
		jlong sonicID, jint newSampleRate) {
	sonicStream stream = getInst(sonicID)->stream;
...
jint Java_fm_player_mediaplayer_player_Sonic_getNumChannelsNative(JNIEnv *env, jobject thiz,
		jlong sonicID) {
	sonicStream stream = getInst(sonicID)->stream;
...
void Java_fm_player_mediaplayer_player_Sonic_setNumChannelsNative(JNIEnv *env, jobject thiz,
		jlong sonicID, jint newNumChannels) {
	sonicStream stream = getInst(sonicID)->stream;
...
jfloat Java_fm_player_mediaplayer_player_Sonic_getSpeedNative(JNIEnv *env, jobject thiz,
		jlong sonicID) {
	sonicStream stream = getInst(sonicID)->stream;
	LOGV("Reading speed");
...
// Change the speed.
void Java_fm_player_mediaplayer_player_Sonic_setSpeedNative(JNIEnv *env, jobject thiz, jlong sonicID,
		jfloat newSpeed) {
	sonicStream stream = getInst(sonicID)->stream;
...
jfloat Java_fm_player_mediaplayer_player_Sonic_getVolumeNative(JNIEnv *env, jobject thiz,
		jlong sonicID) {
	sonicStream stream = getInst(sonicID)->stream;
	LOGV("Reading volume");
...
// Change the volume.
void Java_fm_player_mediaplayer_player_Sonic_setVolumeNative(JNIEnv *env, jobject thiz,
		jlong sonicID, jfloat newVolume) {
	sonicStream stream = getInst(sonicID)->stream;
...
jboolean Java_fm_player_mediaplayer_player_Sonic_getChordPitchNative(JNIEnv *env, jobject thiz,
		jlong sonicID) {
	sonicStream stream = getInst(sonicID)->stream;
...
void Java_fm_player_mediaplayer_player_Sonic_setChordPitchNative(JNIEnv *env, jobject thiz,
		jlong sonicID, jboolean useChordPitch) {
	sonicStream stream = getInst(sonicID)->stream;
...
// object
jint Java_fm_player_mediaplayer_player_Sonic_availableBytesNative(JNIEnv *env, jobject thiz,
		jlong sonicID) {
	sonicStream stream = getInst(sonicID)->stream;
...
void Java_fm_player_mediaplayer_player_Sonic_flushNative(JNIEnv *env, jobject thiz, jlong sonicID) {
	sonicStream stream = getInst(sonicID)->stream;
	LOGV("Flushing stream");
	sonicFlushStream(stream);
}

jbyteArray Java_fm_player_mediaplayer_player_Sonic_shortToByte(JNIEnv *env, jobject obj,
		jshortArray input) {
	...
	(*env)->ReleaseShortArrayElements(env, input, input_array_elements,
			JNI_ABORT);
